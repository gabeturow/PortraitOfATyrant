...
public static class Utilities {
	...
	public static T ForceGetComponent<T>(this GameObject g) where T : Component{
...
//        get { return Time.timeScale == 0 ? Time.deltaTime : Time.deltaTime / Time.timeScale; }
	...
	public static Vector2 SimpleHarmonicMotion(Vector2 current, Vector2 target, ref Vector2 velocity, float angularFrequency, float dampingRatio){
...
public static class DrawArrow
{
	...
	public static void Connector(Vector3 pos, Vector3 direction, float arrowHeadLength = .5f, float arrowHeadAngle = 20f){
        Gizmos.DrawRay (pos, direction);
        DrawArrowEnd(true, pos, direction/1.9f, Gizmos.color, arrowHeadLength, arrowHeadAngle);
	}
    ...
    private static void DrawArrowEnd (bool gizmos, Vector3 pos, Vector3 direction, Color color, float arrowHeadLength = 0.25f, float arrowHeadAngle = 20.0f)
    {
		...
		Vector3 headPos = pos+direction;
        if (gizmos) {
            Gizmos.color = color;
			Gizmos.DrawRay (headPos, right * arrowHeadLength);
			Gizmos.DrawRay (headPos, left * arrowHeadLength);
			Gizmos.DrawRay (headPos, up * arrowHeadLength);
			Gizmos.DrawRay (headPos, down * arrowHeadLength);
        } else {
			Debug.DrawRay (headPos, right * arrowHeadLength, color);
			Debug.DrawRay (headPos, left * arrowHeadLength, color);
			Debug.DrawRay (headPos, up * arrowHeadLength, color);
			Debug.DrawRay (headPos, down * arrowHeadLength, color);
        }
    }
}
