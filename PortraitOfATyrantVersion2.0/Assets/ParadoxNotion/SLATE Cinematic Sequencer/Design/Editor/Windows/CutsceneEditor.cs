...
namespace Slate{
	...
	public class CutsceneEditor : EditorWindow{
		...
		private float? clipScalingGuideTime;
		...
		void OnGUI(){
			...
			leftRect          = new Rect(0, toolbarHeight + topMargin, leftMargin, Screen.height - toolbarHeight - topMargin + scrollPos.y);
			centerRect        = new Rect(leftMargin, topMargin + toolbarHeight, Screen.width - leftMargin - rightMargin, Screen.height - toolbarHeight - topMargin + scrollPos.y);
			...
			if (e.button == 0 && e.type == EventType.MouseDown && GUIUtility.hotControl == 0){
			...
			Handles.color = Color.black; //cheap ver/hor seperators
		...
		void ShowPlaybackControls(Rect topLeftRect){
			...
			if (autoKeyRect.Contains(Event.current.mousePosition)){AddCursorRect(autoKeyRect, MouseCursor.Link);}
			...
			// GUI.Box( Rect.MinMaxRect(autoKeyRect.xMax + 4, topLeftRect.y + 4, topLeftRect.xMax - 4, topLeftRect.y + 4 + 32), "" ,Styles.clipBoxStyle);
			// GUI.backgroundColor = Color.white;
		...
		void ShowTimeInfo(Rect topMiddleRect){
			...
			GUI.color = new Color(1,1,1,0.1f);
			GUI.DrawTexture( Rect.MinMaxRect(0, topMargin-1, topMiddleRect.xMax, topMargin), Styles.whiteTexture);
			GUI.color = Color.white;
			...
			if (centerRect.width / (viewTime/timeStep) > 6){
				for (var i = start; i <= end; i += timeStep){
					...
					var frameRect = Rect.MinMaxRect(posX-1, topMargin-2, posX+1, topMargin-1 );
			...
			for (var i = start; i <= end; i += timeInterval){
				...
				GUI.color = isProSkin? Color.white : Color.black;
				var markRect = Rect.MinMaxRect(posX - 2, topMargin-3, posX + 2, topMargin-1);
		...
		void ShowListGroups(Event e, ref float nextYPos){
			...
			for (int g= 0; g < cutscene.groups.Count; g++){
				...
				//draw vertical graphic on left side of group rects
				if (!group.isCollapsed){
					GUI.color = groupSelected? listSelectionColor : groupColor;
		...
		void ShowListTracks(Event e, CutsceneGroup group, ref float nextYPos){
			...
			for (int t= 0; t < group.tracks.Count; t++){
				...
				var yPos      = nextYPos;
				var trackRect = new Rect(10, yPos, leftRect.width - trackRightMargin - 10, track.finalHeight);
				nextYPos += track.finalHeight + trackMargins;
		...
		void ShowTimeLines(Rect centerRect){
			...
			//bg graphic
			var bgRect = Rect.MinMaxRect(centerRect.xMin, topMargin + toolbarHeight + scrollPos.y, centerRect.xMax, Screen.height - toolbarHeight + scrollPos.y);
			...
			for (int g = 0; g < cutscene.groups.Count; g++){
				...
				if (group is DirectorGroup){
					...
					for (var i = 0; i < sections.Count-1; i++){
						...
						if (section1.colorizeBackground){
							GUI.DrawTexture(Rect.MinMaxRect(sectionRect.xMin, sectionRect.yMax+1, sectionRect.xMax, Screen.height + scrollPos.y), whiteTexture);
						}
				...
				if (group.isCollapsed){
					if ( !(group is DirectorGroup) ){
						...
						GUI.DrawTexture(colRect, Styles.whiteTexture);
						GUI.color = Color.white;
					}
					continue;
				}
				...
				for (int t= 0; t < group.tracks.Count; t++){
					...
					for (int a= 0; a < track.actions.Count; a++){
						...
						if (isSelected){
							...
							GUI.color = isProSkin? new Color(0.65f, 0.65f, 1) : new Color(0.1f, 0.1f, 0.1f);
		...
		void DoMultiSelection(){
			...
			if (multiSelectStartPos != null){
				...
				if ( (start - e.mousePosition).magnitude > 10 ){
					...
					r.xMax = Mathf.Min( Mathf.Max(start.x, e.mousePosition.x), Screen.width );
			...
			if (e.rawType == EventType.MouseUp){
				if (bigEnough){
					multiSelection = clipWrappers.Values.Where(b => AEncapsulatesB(r, b.rect) ).ToList();
		...
		class ActionClipWrapper{
			...
			public void OnMouseUp(){
				...
				isScalingEnd = false;
				CutsceneEditor.current.clipScalingGuideTime = null;
			}
		}
		...
		void ActionClipWindow(int id){
			...
			var dopeHeight = ActionClip.CLIP_DOPESHEET_HEIGHT;
			var dragRect = new Rect( (allowScaleIn? scaleRectWidth : 0), 0, (isScalable? rect.width - (allowScaleIn? scaleRectWidth*2 : scaleRectWidth ): rect.width), rect.height - (hasActiveParameters? dopeHeight : 0) );
			this.AddCursorRect(dragRect, MouseCursor.Link);

			var controlRectIn = new Rect(0, 0, scaleRectWidth, rect.height - (hasActiveParameters? dopeHeight : 0) );
			var controlRectOut = new Rect(rect.width -scaleRectWidth, 0, scaleRectWidth, rect.height - (hasActiveParameters? dopeHeight : 0) );
			if (isScalable){
			...
			if (wrapper.isScalingStart){
				...
				clipScalingGuideTime = action.startTime;
			}
			...
			if (wrapper.isScalingEnd){
				...
				clipScalingGuideTime = action.endTime;
			}
			...
			if (e.type == EventType.MouseDown){
				...
				if (e.control){
					...
					if (multiSelection.Contains(wrapper)){
						multiSelection.Remove(wrapper);
					} else {
						multiSelection.Add(wrapper);
					}
				} else {
