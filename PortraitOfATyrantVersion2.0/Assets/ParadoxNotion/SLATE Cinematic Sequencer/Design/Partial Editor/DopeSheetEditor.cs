...
namespace Slate{
	...
	public static class DopeSheetEditor{
		...
		//Refresh dopesheet keys of target animatable. TODO: Find a better way to handle this by using only the interfaces
		public static void RefreshDopeKeysOf(IAnimatableData animatable){
			...
			if (animatable is AnimationDataCollection){
				foreach(var animParam in ((AnimationDataCollection)animatable).animatedParameters ){
			...
			if (animatable is AnimatedParameter){
				foreach(var pair in cache){
					if (pair.Key is AnimationDataCollection){
						var data = pair.Key as AnimationDataCollection;
						if (data.animatedParameters.Contains( (AnimatedParameter)animatable) ){
		...
		class DopeSheetRenderer {
			...
			void DoVoidContextMenu(Event e){
				...
				var menu = new GenericMenu();
				menu.AddItem(new GUIContent("Create New Key"), false, ()=> { CreateNewKey(cursorTime); });
			...
			void CreateNewKey(float time){
				animatable.TryKeyIdentity(keyable.animatedParametersTarget, time);
				...
				for (var i = 0; i < allKeys.Count; i++){
					...
					if ( !currentTimes.Any( t => Mathf.Abs(t - key.time) <= PROXIMITY_TOLERANCE )){
						...
						foreach(var otherKey in allKeys.Where(k => k.time == key.time )){
							var otherKeyTangent = KeyframeUtility.GetKeyTangentMode(otherKey, 1);
