...
using DepthOfField = UnityStandardAssets.CinematicEffects.DepthOfField;
...
namespace Slate{
	...
	public class DirectorCamera : MonoBehaviour, IDirectableCamera {
		...
		public static bool dontDestroyOnLoad{
			get {return PlayerPrefs.GetInt(DONT_DESTROY_KEY, 1) == 1;}
			set {PlayerPrefs.SetInt(DONT_DESTROY_KEY, value? 1 : 0);}
		}
		...
		public static void Disable(){
			...
			//disable behaviours
			current.cam.gameObject.SetActive(false);
		...
		public static void Update(IDirectableCamera source, IDirectableCamera target, EaseType interpolation, float weight, float damping = 3f){
			...
			if (!isCut && Application.isPlaying && damping > 0){
				...
				current.focalPoint  = Mathf.Lerp(current.focalPoint, targetFocalPoint, Time.deltaTime * damping);
				current.focalRange  = Mathf.Lerp(current.focalRange, targetFocalRange, Time.deltaTime * damping);
		...
		void OnValidate(){
			if (!hasUpdate_152){
				hasUpdate_152 = true;
				foreach(var behaviour in current.cam.gameObject.GetComponents<Behaviour>()){
