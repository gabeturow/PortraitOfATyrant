...
namespace Slate{
	...
	[Description("Camera Shots can be keyframed directly within this clip. You don't need to create an Actor Group to animate the shot. (The SteadyCam Effect is only visible in playmode)")]
	public class CameraShot : DirectorActionClip {
		...
		protected override void OnAfterValidate(){
			...
			var isAlembicCam = false;
			#if !NO_UTJ
			isAlembicCam = targetShot != null && targetShot.gameObject.GetComponent<UTJ.AlembicCamera>() != null;
			#endif

			var useExernalClip = shotAnimationMode == ShotAnimationMode.UseExternalAnimationClip && externalAnimationClip != null;
			SetParameterEnabled("position", !useExernalClip && !isAlembicCam);
			SetParameterEnabled("rotation", !useExernalClip && !isAlembicCam);
			SetParameterEnabled("fieldOfView", !useExernalClip && !isAlembicCam);
		}
