...
namespace Slate{
	...
	abstract public class CutsceneTrack : MonoBehaviour, IDirectable{
		...
		protected int inspectedParameterIndex = -1;
		...
		public float finalHeight{
			...
			{
				if (showCurves){
					return inspectedParameterIndex == -1? Mathf.Max(proposedHeight, defaultHeight + 50 ) : Mathf.Max(customHeight, proposedHeight);
				}
		...
		public void DeleteAction(ActionClip action){
			...
			if (CutsceneUtility.selectedObject == action){
		...
		protected void DoParamsInfoGUI(Event e, Rect trackRect, IKeyable keyable, bool showAddPropertyButton){
			...
			if ((keyable == null || keyable.parent != this) && keyable != this){
			...
			if (keyable.animationData != null && keyable.animationData.animatedParameters != null){
				...
				for (var i = 0; i < paramsCount; i++){
					...
					if (animParam.enabled){
						...
						GUI.enabled = animParam.HasAnyKey() || animParam.isExternal;
						...
						if (GUI.Button(paramRect, string.Empty, GUIStyle.none)){
							...
							CurveEditor.FrameAllCurves(animParam);
						}
					}
				}
			...
			if ( showAddPropertyButton && inspectedParameterIndex == -1){
				...
				if (GUI.Button(buttonRect, "Add Property")){
					EditorTools.ShowAnimatedPropertySelectionMenu(go, AnimatedParameter.supportedTypes, (p, t)=>{
						keyable.animationData.TryAddParameter(p, t, go.transform);
					});
				}
				GUI.enabled = true;
		...
		protected void DoClipCurves(Event e, Rect posRect, Rect timeRect, System.Func<float, float> TimeToPos, IKeyable keyable){
			...
			//this basicaly means that clip is out of view
			if (finalPosRect.width < 0){
				return;
			}
			...
			//just a vertical guide from params to dopesheet
