...
namespace Slate{
	...
	abstract public class ActionClip : MonoBehaviour, IDirectable, IKeyable {
		...
		///Get the AnimatedParameter of name. The name is usually the same as the field or property name that [AnimatableParameter] is used on.
...
#if UNITY_EDITOR
		//Try record keys
		bool IKeyable.TryRecordKeys(float time){
			if (hasParameters && CutsceneUtility.selectedObject == this){
				return animationData.TryKeyChangedValues(animatedParametersTarget, time);
			}
			return false;
		}
#endif
		...
		public const float CLIP_DOPESHEET_HEIGHT = 13f;
		...
		void TryRetime(bool retime, float preScaleStartTime, float preScaleEndTime){
			...
			//notify dopesheet editor refresh dopekeys
			DopeSheetEditor.RefreshDopeKeysOf(animationData);
		}
